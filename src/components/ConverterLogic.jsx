...
const Currency = () => {
  ...
  const [fromNameCurrency, setFromNameCurrency] = useState(
    constans.DEFAULT_FROM_CURRENCY
  );
  const [toNameCurrency, setToNameCurrency] = useState(
    constans.DEFAULT_TO_CURRENCY
  );
  ...
  const getBaseUserGeoCurrency = async () => {
    const v = (await await (await fetch(constans.DEFAULT_URL)).json())
      .currencies[0].code;
    return v;
  };
  ...
  const updateValue = async () => {
    const convertingValue = await convertRates(
      toNameCurrency,
      fromNameCurrency
    );
